...
router.post('/article/create', checkToken, function (req, res, next) {
  api.createArticle(req.body, function (rs) {
...
router.post('/article/lists', checkToken, function (req, res, next) {
  ...
  api.getAllArticles(page, limit, function (result) {
    var articleLists = result
    var total = 0
    api.getArticlesCount(function (rs) {
      if (rs) {
        total = rs
      }
    })
...
router.post('/article/noAuthArtilcelists', function (req, res, next) {
  ...
  api.getArticlesByClassify(classify, function (articleLists) {
    if (articleLists) {
      articleLists.forEach(article => {
        delete article.content
...
router.post('/article/articleLists', function (req, res, next) {
  ...
  api.getAllArticles(page, limit, function (result) {
    if (result) {
      ...
      var hasNext = totalPage > page ? 1 : 0
      var hasPrev = page > 1
      articleLists.forEach(article => {
        delete article.content
        article.contentToMark = article.contentToMark.match(/<p>([\s\S]*?)<\/p>/g)[0]
...
router.post('/article/onePage', checkToken, function (req, res, next) {
  let {id} = req.body
  api.getOneArticle(id, function (oneArticle) {
    if (oneArticle) {
      res.send({
        code: 200,
        oneArticle
...
router.post('/article/remove', checkToken, function (req, res, next) {
  api.removeOneArticle(req.body.id, function (rs) {
    if (rs && rs.affectedRows > 0) {
      res.send({
        code: 200,
        message: '删除成功'
...
router.post('/article/edit', checkToken, function (req, res, next) {
  ...
  api.updateArticle(id, {'classify': classify, 'title': title, 'content': content, 'contentToMark': contentToMark}, function (rs) {
    if (rs && rs.affectedRows > 0) {
      res.send({
        code: 200,
        message: '编辑成功'
